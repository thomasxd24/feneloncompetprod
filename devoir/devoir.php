...
if (isset($_GET['d'])) {
    $d = $_GET['d'];
    $sql = " SELECT eleve.eleveid,eleve.nom,eleve.prenom,devoir.class,GROUP_CONCAT(devoir.note ORDER BY devoir.comid separator '|') as 'note' FROM devoir INNER JOIN matiere ON matiere.matiereid=devoir.matiereid INNER JOIN eleve ON eleve.eleveid=devoir.eleveid WHERE devoir.wholedevoirid='$d' GROUP BY eleve.eleveid,devoir.class ORDER BY eleve.nom ASC";
   //echo $sql;
